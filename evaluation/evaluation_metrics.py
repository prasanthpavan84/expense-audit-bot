...
class EvaluationMetrics:
    ...
    def calculate_policy_accuracy(predictions: List[Dict[str, Any]], targets: List[Dict[str, Any]]) -> float:
        ...
        for p, t in zip(predictions, targets):
            ...
            if p_val is not None and t_val is not None:
                # Normalize values
                p_norm = "approved" if "approve" in str(p_val).lower() else "denied" if "deni" in str(p_val).lower() else "review"
                t_norm = "approved" if "approve" in str(t_val).lower() or str(t_val).lower() == "true" else "denied" if "deni" in str(t_val).lower() or str(t_val).lower() == "false" else "review"
    ...
    def calculate_fraud_accuracy(predictions: List[Dict[str, Any]], targets: List[Dict[str, Any]]) -> float:
        ...
        for p, t in zip(predictions, targets):
            ...
            # Normalize targets
            t_bool = bool(t_val) if isinstance(t_val, bool) or str(t_val).lower() in ["true", "false"] else int(t_val) > 0
