...
def run_metrics_engine():
    ...
    for r in results:
        ...
        if "reason" in cat:
            ...
            if passed: subsystems["reasoning"]["correct"] += 1
            
        # Default reflection, report and workflow evaluations
        subsystems["reflection"]["total"] += 1
        if passed: subsystems["reflection"]["correct"] += 1
    ...
    ocr_acc = subsystems["extraction"]["correct"] / subsystems["extraction"]["total"] if subsystems["extraction"]["total"] > 0 else 1.0
    policy_acc = subsystems["policy"]["correct"] / subsystems["policy"]["total"] if subsystems["policy"]["total"] > 0 else 1.0
    security_acc = subsystems["validation"]["correct"] / subsystems["validation"]["total"] if subsystems["validation"]["total"] > 0 else 1.0
    ...
    metrics = {
        ...
        "subsystems": {
            ...
            "fraud": subsystems["fraud"]["correct"] / subsystems["fraud"]["total"] if subsystems["fraud"]["total"] > 0 else 1.0,
            "policy": policy_acc,
            "reasoning": subsystems["reasoning"]["correct"] / subsystems["reasoning"]["total"] if subsystems["reasoning"]["total"] > 0 else 1.0,
            "reflection": subsystems["reflection"]["correct"] / subsystems["reflection"]["total"] if subsystems["reflection"]["total"] > 0 else 1.0,
            "report": subsystems["report"]["correct"] / subsystems["report"]["total"] if subsystems["report"]["total"] > 0 else 1.0,
            "workflow": subsystems["workflow"]["correct"] / subsystems["workflow"]["total"] if subsystems["workflow"]["total"] > 0 else 1.0
        },
    ...
    # Calculate scorecard
    overall_score = (metrics["subsystems"]["extraction"] * 20) + (metrics["subsystems"]["policy"] * 40) + (metrics["subsystems"]["fraud"] * 20) + (metrics["subsystems"]["validation"] * 20)
