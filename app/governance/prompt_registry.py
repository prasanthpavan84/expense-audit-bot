...
class PromptRegistry:
    ...
    def validate_prompt_model_compatibility(cls, agent_name: str, model_name: str):
        ...
        if model_name not in compatible_models:
            raise ValueError(f"Model '{model_name}' is not compatible with prompt '{agent_name}' ({active_ver}). Compatible models: {compatible_models}")
        ...
        if details.get("requires_structured_output"):
            if not ModelRegistry.check_capabilities(model_name, ["structured_output"]):
                raise ValueError(f"Model '{model_name}' lacks capability 'structured_output' required by prompt '{agent_name}' ({active_ver}).")
