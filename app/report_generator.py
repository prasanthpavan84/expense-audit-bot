...
def generate_markdown_report(audit_result: dict[str, Any]) -> str:
    ...
    if overall_score < 70:
        md.append(
            "* ⚠️ **High Audit Failure Rate**: Recommend manager review for this batch of claims due to low compliance score."
        )
    if fraud_alerts:
        md.append(
            "* 🔍 **Investigate Flags**: Audit team should review the receipts for flagged duplicate claims or weekend purchases."
        )
    if total_rejected > 0:
        md.append(
    ...
    md.append("* Audit run completed at: 2026-06-30T09:12:00Z")
    md.append("* Audit Engine Version: v2.0.0-deterministic")
    md.append("* System Status: Audit completed without processing errors.")
    md.append("")
...
def generate_pdf_report(audit_result: dict[str, Any]) -> bytes:
    ...
    try:
        ...
        story.append(
            Paragraph(
                "Deterministic Multi-Agent Compliance and Anomaly Assessment",
        ...
        metadata_data = [
            [
                Paragraph("Document Type", meta_label_style),
                Paragraph("Compliance Audit Summary", meta_val_style),
            ],
            [
                Paragraph("Prepared For", meta_label_style),
                Paragraph("Finance & Audit Committee", meta_val_style),
            ],
            [
                Paragraph("Prepared By", meta_label_style),
                Paragraph("Expense Audit Multi-Agent Bot", meta_val_style),
            ],
            [
                Paragraph("Date of Run", meta_label_style),
                Paragraph("2026-06-30T09:12:00Z", meta_val_style),
            ],
            [
                Paragraph("Audit Engine Version", meta_label_style),
                Paragraph("v2.0.0-deterministic", meta_val_style),
            ],
            [
                Paragraph("Overall Compliance Score", meta_label_style),
                Paragraph(f"{compliance_score:.1f}%", meta_val_style),
            ],
        ]
        ...
        story.append(
            Paragraph(
                "Confidential — Internal Corporate Governance Audit Records Only",
                footer_style,
            )
        )
        ...
        story.append(
            Paragraph(
                f"Total Claimed Amount: {currency} {audit_result.get('total_claimed', 0.0):,.2f}",
                body_style,
            )
        )
        story.append(
            Paragraph(
                f"Total Reimbursable Amount: {currency} {audit_result.get('total_reimbursable', 0.0):,.2f}",
                body_style,
            )
        )
        story.append(
            Paragraph(
                f"Total Rejected Amount: {currency} {audit_result.get('total_rejected', 0.0):,.2f}",
                body_style,
            )
        )
