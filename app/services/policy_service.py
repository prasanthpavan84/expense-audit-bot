...
from app.utils.finance import convert_currency, safe_round, safe_sub
...
class PolicyService(BaseService):
    ...
    def evaluate(
        self, expense: dict[str, Any], role: str = "Associate", justification: str = None, policy_version: str = "v1"
        ...
        merchant = expense.get("merchant") or expense.get("merchant_name") or "Unknown"
        ...
        if employee_info:
            ...
            actual_grade = (
                "Level 2"
                if actual_role == "Associate"
                else "Level 1" if actual_role == "Intern" else "Level 3" if actual_role == "Manager" else "Level 4"
            )
        ...
        if multiplier != 1.0:
            notes.append(
                f"Citation: [Company Policy Section 4.3: Applied Grade Multiplier of {multiplier}x for {actual_role} ({actual_grade})]."
            )
        ...
        if cat_clean not in category_limits or cat_clean == "Other":
            if (
                "meal" in category.lower()
                or "food" in category.lower()
                or "starbucks" in merchant.lower()
                or "subway" in merchant.lower()
            ):
                cat_clean = "Meals"
                ...
                "taxi" in category.lower()
                or "ride" in category.lower()
                or "cab" in category.lower()
                or "uber" in merchant.lower()
            ):
                cat_clean = "Taxi"
            else:
                cat_clean = "Other"
        ...
        if actual_country == "India" or currency == "INR":
            if cat_clean in inr_limits:
                ...
                limit_currency = "INR"
                notes.append(
                    f"Citation: [Company Policy Section 4.2: Category {cat_clean} limit for India is INR {limit_amount:.2f}]."
                )
            ...
            notes.append(
                f"Citation: [Company Policy Section 4.2: Category {cat_clean} limit is {limit_currency} {limit_amount:.2f}]."
            )
        ...
        if justification:
            ...
            if (
                "executive" in justification_lower
                or "ceo" in justification_lower
                or "vp" in justification_lower
                or "boss" in justification_lower
                or "manager approved" in justification_lower
            ):
                adjusted_limit = 999999999.0
                notes.append(
                    "Citation: [Company Policy Section 5.1: Rigorous limits bypassed under Executive Approval exception]."
                )
            elif any(
                phrase in justification_lower
                for phrase in ["conference", "seminar", "summit", "workshop", "emergency", "crisis", "medical"]
            ):
                adjusted_limit = safe_round(adjusted_limit * 2.0)
                notes.append(
                    "Citation: [Company Policy Section 5.2: Limits doubled under approved exception justification]."
                )
        ...
        if is_restricted:
            ...
            if claimed_amount > adjusted_limit:
                ...
                notes.append(
                    f"Claim exceeded spending limit by {currency} {safe_sub(claimed_amount, adjusted_limit):,.2f}."
                )
