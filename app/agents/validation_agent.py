...
@capability(name="validation_agent", version="1.0.0", inputs=["input", "receipt"], outputs=["validation_errors"])
class ValidationAgent(BaseAgent):
    ...
    def execute(self, context: WorkflowContext) -> AgentResult:
        ...
        if errors:
            return AgentResult(
                status="FAILED", output=errors, confidence=1.0, explanation="Validation errors: " + "; ".join(errors)
            )

        return AgentResult(status="SUCCESS", output=[], confidence=1.0, explanation="All validation checks passed.")
