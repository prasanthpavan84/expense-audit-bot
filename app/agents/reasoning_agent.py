...
@capability(name="reasoning_agent", version="1.0.0", inputs=["receipt", "policy_res"], outputs=["reasoning_res"])
class ReasoningAgent(BaseAgent):
    """Reasoning Agent performs financial calculations and currency conversions."""

    def __init__(self, reasoning_service: ReasoningService | None = None, **kwargs):
    ...
    def execute(self, context: WorkflowContext) -> AgentResult:
        ...
        if not receipt:
            return AgentResult(
                status="FAILED", output=None, confidence=0.0, explanation="Reasoning failed: receipt is missing."
            )
