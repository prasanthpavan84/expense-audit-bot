...
class ReflectionAgent(BaseAgent):
    ...
    async def process_state(self, state: WorkflowState) -> WorkflowState:
        ...
        for idx, exp in enumerate(state.expenses):
            ...
            if res:
                ...
                merchant_ev = [
                    ev for ev in evidence_list if ev.get("field") == "merchant" and ev.get("value") == exp.merchant
                ]
                amount_ev = [
                    ev for ev in evidence_list if ev.get("field") == "amount" and ev.get("value") == exp.amount
                ]
                ...
                if res.fraud_score > 0:
                    ...
                    if fraud_indicators:
                        trace.append(
                            f"Fraud check returned a risk score of {res.fraud_score} based on: {', '.join(fraud_indicators)}."
                        )
                        EvidenceCollector.add(
                            state, "fraud_agent", "fraud_risk", res.fraud_score, 0.9, "fraud_agent", True
                        )
                    else:
                        trace.append(
                            f"[UNSUPPORTED] Fraud check returned a risk score of {res.fraud_score} without clear risk indicators."
                        )
                        EvidenceCollector.add(
                            state, "fraud_agent", "fraud_risk", res.fraud_score, 0.4, "fraud_agent", False
                        )
                ...
                if res.policy_violations:
                    ...
                    for violation in res.policy_violations:
                        ...
                        if violation in policy_meta or "policy" in str(state.metadata):
                            ...
                            EvidenceCollector.add(
                                state, "policy_agent", "policy_violation", violation, 1.0, "policy_agent", True
                            )
                        else:
                            verified_violations.append(f"[UNSUPPORTED] {violation}")
                            EvidenceCollector.add(
                                state, "policy_agent", "policy_violation", violation, 0.5, "policy_agent", False
                            )

                    trace.append(
                        f"Identified {len(res.policy_violations)} policy violations: {', '.join(verified_violations)}."
                    )
                else:
                ...
                if res.is_approved:
                    if has_violations or high_fraud:
                        conclusion = (
                            "[CONTRADICTION] Conclusion: The expense is approved despite Compliancy/Fraud violations."
                        )
                    else:
                ...
                state.metadata["reflection_results"][f"expense_{idx}"] = {
                    "confidence": (
                        0.9
                        if "[CONTRADICTION]" not in conclusion and "[UNSUPPORTED]" not in res.reasoning_trace
                        else 0.5
                    ),
    ...
    def execute(self, context: WorkflowContext) -> AgentResult:
        ...
        return AgentResult(status="SUCCESS", output={}, confidence=1.0, explanation="Reflection completed.")
