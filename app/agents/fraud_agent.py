...
@capability(name="fraud_agent", version="1.0.0", inputs=["receipt"], outputs=["fraud_res"])
class FraudAgent(BaseAgent):
    """Fraud Agent assesses risk and indicators utilizing FraudService."""

    def __init__(
        self, fraud_service: FraudService | None = None, audit_repository: AuditRepository | None = None, **kwargs
    ):
    ...
    def execute(self, context: WorkflowContext) -> AgentResult:
        ...
        if not receipt:
            return AgentResult(
                status="FAILED", output=None, confidence=0.0, explanation="Fraud check failed: receipt is missing."
            )
        ...
        for exp in history_expenses:
            history_dicts.append(
                {"merchant": exp.merchant, "amount": exp.amount, "date": exp.date, "currency": exp.currency}
            )
        ...
        try:
            ...
            return AgentResult(status="FAILED", output=None, confidence=0.0, explanation=f"Fraud check failed: {e!s}")
