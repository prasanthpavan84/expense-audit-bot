...
@capability(name="human_review_agent", version="1.0.0", inputs=["receipt", "fraud_res"], outputs=["review_status"])
    ...
    def execute(self, context: WorkflowContext) -> AgentResult:
        ...
        if needs_review:
            ...
            self.services.event_bus.publish(
                "HighRiskDetectedEvent", {"item_id": item_id, "input": context.input, "fraud_score": fraud_score}
            )
        ...
        return AgentResult(
            status="SUCCESS", output={"status": "APPROVED"}, confidence=1.0, explanation="No human review required."
        )
