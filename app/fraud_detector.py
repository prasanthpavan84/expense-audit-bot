...
def calculate_fraud_score(
    expense: dict[str, Any], history: list[dict[str, Any]] = None, session_items: list[dict[str, Any]] = None
) -> tuple[int, str]:
    """
    ...
    if date_str and date_str.lower() != "unknown":
        try:
            ...
            if dt.weekday() in [5, 6]:  # Saturday = 5, Sunday = 6
                ...
                reasons.append(
                    f"Expense claimed on weekend: {dt.strftime('%A')} (+{weights.get('weekend_anomaly', 15)})"
                )
        except Exception:
            pass
    ...
    if threshold_val * 0.90 <= amount < threshold_val:
        ...
        reasons.append(
            f"Claim amount ({currency} {amount:.2f}) is suspiciously close to review threshold ({currency} {threshold_val}) (+{weights.get('just_under_threshold', 25)})"
        )
    ...
    if history:
        ...
        if exact_matches > 0:
            ...
            reasons.append(
                f"Exact duplicate of {exact_matches} historical claim(s) (+{weights.get('duplicate_claim_history', 30)})"
            )
        if repeated_30days > 0:
            score += weights.get("repeated_merchant_30days", 15)
            reasons.append(
                f"Identical amount submitted for '{merchant}' within 30 days (+{weights.get('repeated_merchant_30days', 15)})"
            )
        if consecutive_split > 0:
            score += weights.get("duplicate_claim_session", 30)
            reasons.append(
                f"Potential split transaction: consecutive/same day claims at '{merchant}' (+{weights.get('duplicate_claim_session', 30)})"
            )
    ...
    if session_items:
        ...
        for other in session_items:
            ...
            if other is expense:
                continue
            if (
                str(other.get("merchant", "")).lower() == merchant.lower()
        ...
        if batch_exact > 0:
            score += weights.get("duplicate_claim_session", 30)
            reasons.append(
                f"Duplicate receipt matching another item in the same request batch (+{weights.get('duplicate_claim_session', 30)})"
            )
            ...
            for past in history:
                ...
                if past_city and past_city != current_city and past_date_str and past_date_str.lower() != "unknown":
                    ...
                    if days_diff <= 1:
                        ...
                        reasons.append(
                            f"Impossible Travel: Claimed in {current_city.capitalize()} and {past_city.capitalize()} within {days_diff} day(s) (+{weights.get('impossible_travel', 35)})"
                        )
                        break
        except Exception:
            pass
