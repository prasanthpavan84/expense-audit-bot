...
def validate_currency(currency: str) -> str | None:
...
def check_hallucination(extracted: dict[str, Any], raw_text: str) -> list[str]:
    ...
    if merchant and merchant.lower() != "unknown" and merchant.lower() != "other":
        ...
        is_exemption = clean_merchant.strip() == "taxi ride" and any(
            kw in clean_text_for_merchant for kw in ["uber", "taxi", "cab", "ride"]
        )
        if not is_exemption and words and not any(word in clean_text_for_merchant for word in words):
            errors.append(
                f"Extracted merchant '{merchant}' was not found in the original text (possible hallucination)."
            )
    ...
    if amount > 0:
        ...
        if amt_str not in clean_text and amt_str_short not in clean_text:
            ...
            if not any(int(n) == int(amount) for n in nums):
                errors.append(
                    f"Extracted amount '{amount}' was not found in the original text (possible hallucination)."
                )

    return errors


def validate_single_expense(
    expense: dict[str, Any], raw_text: str, history: list[dict[str, Any]] = None, session_id: str = None
) -> list[str]:
    errors = []
    if session_id:
        print(
            f"AUDIT_TRACE: [Session: {session_id}] Validating expense: {expense.get('merchant')} - {expense.get('amount')}"
        )
    ...
    if items_list and isinstance(items_list, list):
        ...
        for idx, item in enumerate(items_list):
            ...
            if item_amount <= 0:
                errors.append(
                    f"Invalid amount {item_amount} for item '{item.get('name', f'Item {idx+1}')}'. Must be positive."
                )
            total_items_sum += item_amount
        if amount is not None and abs(total_items_sum - float(amount)) > 0.01:
            errors.append(
                f"Arithmetic mismatch: Total amount {amount} does not equal the sum of itemized details {total_items_sum:.2f}."
            )
    ...
    if history and expense.get("merchant") and expense.get("date") and expense.get("amount"):
        ...
        if merchant_lower != "unknown" and date_str != "unknown":
            for past in history:
                if (
                    str(past.get("merchant")).lower() == merchant_lower
                    and str(past.get("date")) == date_str
                    and abs(float(past.get("amount", 0)) - amt) < 0.01
                ):
                    errors.append(
                        f"Duplicate Claim Error: An identical expense from '{expense.get('merchant')}' on {date_str} for {expense.get('currency')} {amt:.2f} has already been submitted."
                    )
                    break
...
def validate_expenses(
    expenses: list[dict[str, Any]], raw_text: str, history: list[dict[str, Any]] = None, session_id: str = None
) -> list[str]:
    if not expenses:
    ...
    for idx, exp in enumerate(expenses):
        ...
        if merchant and date_val and amount and merchant != "unknown" and date_val != "unknown":
            ...
            if key in seen:
                all_errors.append(
                    f"Expense #{idx+1}: Duplicate claim detected within the same submission batch for merchant '{exp.get('merchant')}' on {date_val} for {amount}."
                )
            seen.add(key)
