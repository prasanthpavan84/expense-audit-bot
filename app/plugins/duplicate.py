    ...
    def check(
        ...
        if history:
            ...
            if consecutive_split > 0:
                score += 30
                reasons.append(f"Potential split transaction: consecutive/same day claims at '{expense.get('merchant')}' (+30)")
        ...
        if session_items:
            ...
            for other in session_items:
                if other is expense:
                    continue
                if (str(other.get("merchant", "")).lower() == merchant and
                    str(other.get("date", "")) == date_str and
                    abs(float(other.get("amount", 0.0)) - amount) < 0.01):
                    batch_exact += 1
            if batch_exact > 0:
                score += 30
                reasons.append(f"Duplicate receipt matching another item in the same batch (+30)")
