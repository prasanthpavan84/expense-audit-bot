...
_RECEIPT_INDICATORS: List[Dict] = [
    ...
    {"name": "payment_method", "pattern": re.compile(r"\b(visa|mastercard|amex|debit|credit|cash|card|upi|paypal)\b", re.I), "weight": 0.08},
...
class ReceiptDetector:
    ...
    def detect(text: str) -> ReceiptDetectionResult:
        ...
        reason = f"Receipt score {score:.2f} from {len(matched)} indicators" if matched else "No receipt indicators found"
