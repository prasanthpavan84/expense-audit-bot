...
class WorkflowEngine:
    ...
    def __init__(
        ...
        checkpointer: Optional[CheckpointExecutor] = None,
    ...
    async def execute_workflow(
        ...
        for step in steps_to_run:
            ...
            self.event_repository.save_event(audit_id, correlation_id, step, "AgentStarted", {"correlation_id": correlation_id})
            ...
            for attempt in range(retry_count):
                try:
                    ...
                    if result.success:
                        ...
                        self.event_repository.save_event(audit_id, correlation_id, step, "AgentCompleted", completed_payload)
                        break
                    else:
                        self.logger.warning(f"Agent '{step}' returned unsuccessful result (Attempt {attempt+1}/{retry_count}): {result.explanation}")
                except Exception as e:
                    self.logger.error(f"Error executing agent '{step}' (Attempt {attempt+1}/{retry_count}): {str(e)}")
            ...
            if not agent_success:
                ...
                # Publish event
                self.event_repository.save_event(audit_id, correlation_id, step, "AgentFailed", {"explanation": fallback_explanation})
