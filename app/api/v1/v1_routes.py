...
class AuditRequest(BaseModel):
    ...
    justification: Optional[str] = Field(None, example="Client meeting")
...
async def replay_audit(audit_id: str):
    ...
    return {
        "audit_id": audit_id,
        "expense": expense.model_dump() if expense else None,
        "timeline": timeline
    }
...
async def get_agents_registry():
    ...
    agent_list = []
    for agent_key in ["planner_agent", "receipt_extractor", "fraud_agent", "policy_agent", "reasoning_agent", "reflection_agent", "report_agent"]:
...
async def get_tools_registry():
    ...
    for tool_name in registered:
        tools.append({
            ...
            "provider": "Filesystem MCP" if "FILE" in tool_name else ("Policy MCP" if "POLICY" in tool_name else "Currency MCP"),
