...
def execute_agent_sync(prompt: str) -> dict:
    ...
    return {"output": full_text.strip(), "state": state}
...
def test_ocr_failure():
    ...
    assert (
        "denied" in str(res["state"]).lower()
        or "rejected" in str(res["state"]).lower()
        or "validation" in str(res["state"]).lower()
    )
...
def test_empty_receipt():
    ...
    assert (
        "rejected" in str(res["state"].get("orchestrator_decision")).lower()
        or "validation" in str(res["state"]).lower()
        or "denied" in str(res["state"]).lower()
    )
...
def test_prompt_injection():
    ...
    assert (
        "security" in res["output"].lower()
        or "blocked" in res["output"].lower()
        or "denied" in str(res["state"].get("orchestrator_decision")).lower()
    )
